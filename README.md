...
| 统一前端（推荐） | `streamlit run manual_test_app.py` | 手动参数/模拟/LLM/GA/GA+LLM 混合优化一体化 |
| 参数化 L 形（UI，旧） | `streamlit run parametric_l_app.py` | 实时调参 + 模拟结果 Tab |
...
### 统一前端入口 — `manual_test_app.py`（后续只维护这里）

**约定：之后项目的前端功能统一维护在 `manual_test_app.py`。**

- 手动设置建筑参数（当前默认使用 `scripts/generate_20260528.py`）
- 3D 模型预览（Plotly）
- EnergyPlus（direct 路径）模拟与结果可视化
- LLM 优化（过程 + 最优方案 + 3D 分区能耗）
- GA 优化（过程曲线 + 最优方案）
- **GA+LLM 混合优化**（Tab「GA+LLM 混合优化」→ **一键启动**，预设快速冒烟/标准/自定义）

启动命令：

```bash
# 推荐（使用 uv 管理依赖）
uv run streamlit run manual_test_app.py

# 或直接运行（确保已安装依赖：streamlit/plotly 等）
streamlit run manual_test_app.py
```
...
├── manual_test_app.py           # ✅ 统一前端入口（后续主要维护这里）
